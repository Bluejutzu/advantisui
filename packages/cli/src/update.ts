...
import { downloadComponent } from "./components.js";

export async function checkForUpdates(outDir: string) {
  const metadata = loadMetadata(outDir);
  const results: {
    component: string;
    status: "outdated" | "modified" | "up-to-date";
  }[] = [];

  const files = fs
    .readdirSync(outDir)
    .filter((f) => f.endsWith(".tsx") && f !== METADATA_FILE);
  if (files.length === 0) {
    console.log(
      chalk.yellow("⚠️ No components found in the output directory."),
    );
    return results;
  }

  console.log(
    chalk.cyan(`🔍 Checking ${files.length} component(s) for updates...\n`),
  );
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (!remoteHash) {
        console.log(
          chalk.yellow(
            `⚠️ ${componentName}: Could not fetch remote version (may have been removed)`,
          ),
        );
        continue;
      }
      ...
      console.log(
        chalk.red(`❌ Error checking ${componentName}: ${err.message}`),
      );
    }
  }
  ...
  for (const component of componentsToUpdate) {
    ...
    try {
      ...
      metadata[component] = {
        ...
        installedAt:
          metadata[component]?.installedAt || new Date().toISOString(),
      ...
      console.error(
        chalk.red(`❌ Failed to update ${component}: ${err.message}`),
      );
...
export async function handleUpdateCommand(config: Config) {
  ...
  if (!fs.existsSync(outDir)) {
    console.log(
      chalk.red(`❌ Component directory "${config.outDir}" does not exist.`),
    );
    process.exit(1);
  }
  ...
  if (outdated.length === 0 && modified.length === 0) {
    ...
    console.log(
      chalk.yellow(
        `Please be aware that certain diff's may not be detected. ${chalk.gray("I.e")} ${chalk.red("hover:scale-105")} | ${chalk.green("hover:scale-107")}`,
      ),
    );
    return;
  }
  ...
  const { updateChoice } = await inquirer.prompt<{
    updateChoice: "all" | "select" | "force" | "none";
  }>([
    {
      type: "list",
      name: "updateChoice",
    ...
    const { selected } = await inquirer.prompt<{ selected: string[] }>([
      {
        ...
        name: "selected",
        message:
          "Select components to force update (will overwrite local changes):",
